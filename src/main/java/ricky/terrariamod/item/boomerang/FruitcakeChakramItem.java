...
import ricky.terrariamod.entity.weapon.FruitcakeChakramEntity;
import ricky.terrariamod.item.AttackableItem;

public class FruitcakeChakramItem extends Item implements AttackableItem {
    public FruitcakeChakramItem(Settings settings) {
        super(settings);
    }
    ...
    public void attack(World world, PlayerEntity playerEntity, Hand hand) {
        if (!world.isClient) {
            ...
            // FruitcakeChakramEntity を生成
            FruitcakeChakramEntity boomerangEntity = new FruitcakeChakramEntity(world, playerEntity, boomerang);
