...
public class TrimarangEntity extends PersistentProjectileEntity {
    ...
    public TrimarangEntity(EntityType<? extends PersistentProjectileEntity> entityType, World world) {
        super(entityType, world);
        this.boomerangStack = new ItemStack(ModItems.TRIMARANG); // デフォルトのアイテムスタックを設定
    }

    public TrimarangEntity(World world, LivingEntity owner, ItemStack stack) {
        super(ModEntities.TRIMARANG, owner, world);
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        if (!returning) {
            if (entityHitResult.getEntity() != this.getOwner()) {
                ...
                entityHitResult.getEntity().damage(damageSource, 9.5F);  // 5ダメージを与える
