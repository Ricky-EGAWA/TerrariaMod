...
public class CorruptSlimeOverlayFeatureRenderer<T extends LivingEntity>
        extends FeatureRenderer<T, CorruptSlimeModel<T>> {
    private final EntityModel<T> model;

    public CorruptSlimeOverlayFeatureRenderer(FeatureRendererContext<T, CorruptSlimeModel<T>> context, EntityModelLoader loader) {
        super(context);
        this.model = new CorruptSlimeModel<>(loader.getModelPart(EntityModelLayers.SLIME_OUTER));
    }
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        ...
        ((CorruptSlimeModel)this.getContextModel()).copyStateTo(this.model);
